<template>
  <div class="o-sldie-container" :style="{width:option.slideViewWidth + 'px'}">
    ...
    <div class="only-slide" :style="{height:option.slideViewHeight + 'px'}" @mouseover="hoverPrevNextVisible"
    ...
    <div class="o-preview">
      <div class="o-preview-wapper">
        <ul :style="{left:previewLeft+'px'}" ref="hidebox">
          ...
          <li v-for="(item,index) in option.slideData[caIdx].photo" @click="clickPreview(index)"
              :style="{opacity:cpIdx==index?1:.4}"><img :src="item.img" alt=""></li>
          <li class="cut-album-btn" v-if="caIdx==option.slideData.length - 1?false:true"
              @click="cutAlbumInPreview('next')">下一图集
          </li>
        </ul>
      </div>
